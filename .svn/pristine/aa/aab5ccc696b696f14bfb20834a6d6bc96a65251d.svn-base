  ...
  <div>
    ...
    <div class="qs-info" v-for="info in question.info" :key="info.info_id">
      <div class="info-content">
        请听短文：<span v-show="info.source_content" class="horn-icon" :data-src="info.source_content" @click="audioPlay($event)"></span>
      </div>
      ...
      <div class="items" v-for="item in info.items" :key="item.item_id">
        <div class="item-content">{{item.item_content}} <span v-show="item.source_content" class="horn-icon" :data-src="item.source_content" @click="audioPlay($event)"></span></div>
        <div class="item-answer" v-for="(answer,index) in item.answers" :key="answer.answer_id">
          {{options[index]}}. {{answer.answer_content}}
