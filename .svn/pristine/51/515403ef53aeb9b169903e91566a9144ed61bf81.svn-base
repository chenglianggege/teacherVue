<template>
  <div>
    <div class="qs-title">{{question.qs_title}}<span class="fr">
          <el-button
            v-show="showBtn"
            :type="cart.hasOwnProperty('qsId_' + question.qs_id) ? 'success' : 'primary'" size="mini"
    ...
    <el-checkbox v-if="showBtn"  :disabled="cart.hasOwnProperty('qsId_' + question.qs_id) ? true : false" :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
    <el-row v-if="showBtn">
      <el-checkbox-group  :disabled="cart.hasOwnProperty('qsId_' + question.qs_id) ? true : false" v-model="checkedItems" @change="handleCheckedItemsChange">
        <el-col :span="24" v-for="info in question.info" :key="info.info_id">
