  ...
  <div>
    ...
    <div class="qs-info" v-for="info in question.info" :key="info.info_id">
      ...
      <div class="items" v-for="item in info.items" :key="item.item_id">
        <div class="item-content"><span v-html="coverToBrTag(item.item_content)"></span> <span v-show="item.source_content" class="horn-icon" :data-src="item.source_content" @click="audioPlay($event)"></span></div>
