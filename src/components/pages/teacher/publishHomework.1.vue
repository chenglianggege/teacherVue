    ...
    <el-main  class="container"  v-if="!fullscreenLoading">
      <div class="c-body">
        <div class="homework-div">
          <div class="sel-level">
            <ul class="fl">
              <li @click="currentLevel = key" v-for="(level, key) in allLevel" :class="currentLevel == key ? 'li-on' : ''" :key="key">{{level}}</li>
            </ul>
          ...
          <div class="sel-condition" style="display: block" v-show="books.length > 1">
            <div class="sel-grade">
              <div class="sel-m-box">
                <span class="sel-title">教材:</span>
                <ul>
                  <li @click="currentBookPress = key" v-for="(press, key) in books" :key="key" :class="{'li-on' :currentBookPress==key}">
                    <span>{{press.press_name}}</span>
                  </li>
                </ul>
                <div class="cl-fix"></div>
              </div>
          ...
          <div class="sel-condition" style="display: block" v-if="books.length">
            <div class="sel-grade">
              <div class="sel-m-box">
                <span class="sel-title">年级:</span>
                <ul>
                  <li @click="setCurrentBook(book)" v-for="book in books[currentBookPress].books" :key="book.book_id" :class="{'li-on' :currentBook.book_id==book.book_id}">
          ...
          <el-container class="unit-box">
            <el-aside width="300px"  v-loading.body.lock="loadingPaper" style="background-color: rgb(238, 241, 246)">
              <el-menu v-show="papersList && papersList.length" @open="getQuestion" style="margin-right: 2px">
                <el-submenu style="text-overflow: ellipsis;overflow: hidden;" v-for="paper in papersList" :key="paper.paper_id" :index="paper.paper_id" >
                  ...
                  <el-menu-item @click="questionDetail=question;paperTitle=paper.paper_title"   v-for="question in questionList[paper.paper_id]" :key="question.qs_id" :index="question.qs_id">
                    <i class="el-icon-d-arrow-right"></i><span>{{question.qs_title}}</span>
                  </el-menu-item>
                </el-submenu>
              </el-menu>
...
export default {
  ...
  methods: {
    ...
    async loadBooks () {
      ...
      this.books = []
      try {
        let books = await axios.get(window.API_URL + '/v2/teacher/paper/books',
          {params: {token: token, study_level: +this.currentLevel + 1}})
        if (books.data.retCode === 0) {
          this.books = books.data.retData ? books.data.retData : []
        } else {
          this.books = []
        }
        this.currentBookPress = 0
        if (this.books.length && this.books[this.currentBookPress].books.length) {
          this.currentBook = this.books[this.currentBookPress].books[0]
        }
      } catch (e) {
        console.log(e)
      } finally {
    ...
    async loadPaper () {
      ...
      try {
        let papersList = await axios.get(window.API_URL + '/v2/teacher/paper/query',
          {params: {token: token, book_id: this.currentBook.book_id, paper_type: this.currentModule !== 4 ? this.currentModule : this.currentModule2, qs_type: this.currentQsType}})
    ...
    async checkClass () {
      ...
      if (teacherInfo.data.retCode === 0) {
        ...
        if (parseInt(teacherInfo.class_num) === 0) {
          let _this = this
          this.$alert('还未创建班级，请前往班级管理创建班级后再布置作业！', '提示', {confirmButtonText: '确定'}).then(function () {
    ...
    async setCurrentBook (currentBook) {
      this.currentBook = currentBook
      if (+this.currentGrade === +this.currentBook.book_grade) {
      ...
      this.currentGrade = this.currentBook.book_grade
    }
  },
  watch: {
    currentLevel () {
